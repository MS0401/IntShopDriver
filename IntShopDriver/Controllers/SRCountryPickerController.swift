...
protocol CountrySelectedDelegate {
    func SRcountrySelected(countrySelected country: Country, flagImage: UIImage) -> Void
}
...
extension SRCountryPickerController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if checkSearchBarActive() {
      ...
      countryDelegate.SRcountrySelected(countrySelected: countriesFiltered[indexPath.row], flagImage: image!)
    }else {
      contry = countriesModel[indexPath.row]
      let imagestring = contry.country_code
      let imagePath = "CountryPicker.bundle/\(imagestring).png"
      let image = UIImage(named: imagePath)
      countryDelegate.SRcountrySelected(countrySelected: countriesModel[indexPath.row], flagImage: image!)
    }
